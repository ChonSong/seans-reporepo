# sean's-reporepo

> Personal code catalog — owned and starred repositories indexed for ideation, discovery, and combinatorial application design.

## Quick Stats

| | Owned | Starred | Total |
|---|---|---|---|
| Repos | 41 | 147 | 188 |
| Total Size | 2,025,665 KB | 15,919,311 KB | 17,944,976 KB (17,524 MB) |
| Languages | 11 | 16 | 19 |
| Tags | — | — | 67 |

## Structure

```
├── owned/           # My repositories (41)
│   ├── agent-os.md
│   ├── repo-transmute.md
│   └── ...
├── starred/         # Repos I've starred (147)
│   ├── yamadashy_repomix.md
│   ├── e2b-dev_awesome-ai-agents.md
│   └── ...
├── scripts/         # Utility scripts for catalog management
└── README.md        # This file
```

## Owned Repositories

### Agent (14)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [BrowserOS](owned/ChonSong_BrowserOS.md) | C++ | 583,037 KB | 0 | agent, ai, api |
| [g3kilocode](owned/ChonSong_g3kilocode.md) | TypeScript | 501,876 KB | 0 | agent, ai, api |
| [hermes-agent](owned/ChonSong_hermes-agent.md) | Python | 172,173 KB | 0 | agent, ai, automation |
| [nanobot](owned/ChonSong_nanobot.md) | Python | 39,921 KB | 0 | agent, ai, bot |
| [hermes-web-computer](owned/ChonSong_hermes-web-computer.md) | Go | 13,870 KB | 0 | agent, ai, audio |
| [claw-aie-harness](owned/ChonSong_claw-aie-harness.md) | Python | 10,759 KB | 0 | agent, ai, aie |
| [onefilellm](owned/ChonSong_onefilellm.md) | Python | 1,427 KB | 0 | agent, ai, api |
| [Codeovertcp](owned/ChonSong_Codeovertcp.md) | other | 153 KB | 0 | agent, ide, voice |
| [claw-aie](owned/ChonSong_claw-aie.md) | Python | 126 KB | 0 | agent, ai, aie |
| [casaos-agent](owned/ChonSong_casaos-agent.md) | Go | 108 KB | 0 | agent, api, awesome-list |
| [nanobot-workspace](owned/ChonSong_nanobot-workspace.md) | Shell | 101 KB | 0 | agent, bot |
| [ecosystem](owned/ChonSong_ecosystem.md) | Shell | 93 KB | 0 | agent, ai, aie |
| [casaos-webhook-emitter](owned/ChonSong_casaos-webhook-emitter.md) | Go | 21 KB | 0 | agent, ai, api |
| [hermes-bootstrap](owned/ChonSong_hermes-bootstrap.md) | Shell | 6 KB | 0 | agent, ai, browser-automation |

### Cli (2)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [starcraft-battlenet-web](owned/ChonSong_starcraft-battlenet-web.md) | HTML | 7 KB | 0 | ai, api, browser-automation |
| [clonezilla-backup](owned/ChonSong_clonezilla-backup.md) | other | 2 KB | 0 | ai, backup, cli |

### Infrastructure (3)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [mobile-codeovertcp.com](owned/ChonSong_mobile-codeovertcp.com.md) | CSS | 180 KB | 0 | ai, backup, ide |
| [minsky-circuit](owned/ChonSong_minsky-circuit.md) | Python | 4 KB | 0 | ai, infrastructure, python |
| [energy-aware-task-router](owned/ChonSong_energy-aware-task-router.md) | Python | 3 KB | 0 | ai, api, energy |

### Library (9)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [rasta-assistant](owned/ChonSong_rasta-assistant.md) | TypeScript | 37,004 KB | 0 | agent, ai, framework |
| [hermes-webui](owned/ChonSong_hermes-webui.md) | Python | 25,564 KB | 0 | agent, ai, bot |
| [pokerkit](owned/ChonSong_pokerkit.md) | Python | 6,067 KB | 0 | ai, api, automation |
| [repo-transmute](owned/ChonSong_repo-transmute.md) | TypeScript | 4,032 KB | 0 | agent, ai, cli |
| [postflop-solver](owned/ChonSong_postflop-solver.md) | Rust | 547 KB | 0 | ai, gaming, go |
| [seans-reporepo](owned/ChonSong_seans-reporepo.md) | Python | 389 KB | 0 | agent, ai, aie |
| [linux-web-serving-infrastructure](owned/ChonSong_linux-web-serving-infrastructure.md) | TypeScript | 276 KB | 0 | ai, api, automation |
| [hermes-computer-planning](owned/ChonSong_hermes-computer-planning.md) | other | 22 KB | 0 | agent, ai, api |
| [circuit-breaker-framework](owned/ChonSong_circuit-breaker-framework.md) | Python | 15 KB | 0 | agent, ai, aie |

### Monorepo (6)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [hermes-sync](owned/ChonSong_hermes-sync.md) | Python | 43,670 KB | 0 | agent, ai, api |
| [agent-os](owned/ChonSong_agent-os.md) | Python | 10,271 KB | 0 | agent, ai, api |
| [features-list](owned/ChonSong_features-list.md) | HTML | 1,706 KB | 0 | agent, ai, analytics |
| [everything-dashboard](owned/ChonSong_everything-dashboard.md) | TypeScript | 77 KB | 0 | agent, dashboard, express |
| [llm-benchmark-platform](owned/ChonSong_llm-benchmark-platform.md) | Python | 73 KB | 0 | ai, api, awesome-list |
| [seans](owned/ChonSong_seans.md) | other | 5 KB | 1 | agent, ai, aie |

### Unknown (5)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [openclaw-backup](owned/ChonSong_openclaw-backup.md) | Shell | 520,831 KB | 0 |  |
| [sean-dotfiles](owned/ChonSong_sean-dotfiles.md) | QML | 998 KB | 0 | ai, dotfiles, gaming |
| [sean-s-landing-page](owned/ChonSong_sean-s-landing-page.md) | TypeScript | 96 KB | 0 | ai, api, website |
| [ai-system-monitor](owned/ChonSong_ai-system-monitor.md) | Python | 3 KB | 0 |  |
| [openclaw-backup-test](owned/ChonSong_openclaw-backup-test.md) | other | 0 KB | 0 |  |

### Utility (1)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [system-backup](owned/ChonSong_system-backup.md) | Shell | 45,702 KB | 0 | backup |

### Webapp (1)

| Repo | Language | Size | Stars | Tags |
|---|---|---|---|---|
| [wasm-postflop](owned/ChonSong_wasm-postflop.md) | Vue | 4,450 KB | 0 | ai, browser-automation, gaming |

## Starred Repositories (Top 20 by Stars)

| Repo | Type | Language | Stars | Tags |
|---|---|---|---|---|
| [awesome](starred/sindresorhus_awesome.md) | awesome-list | other | 469,802 | awesome-list |
| [openclaw](starred/openclaw_openclaw.md) | agent | TypeScript | 374,464 | ai |
| [developer-roadmap](starred/nilbuild_developer-roadmap.md) | unknown | TypeScript | 355,399 | ide |
| [claw-code](starred/ultraworkers_claw-code.md) | unknown | Rust | 192,430 | rust |
| [ECC](starred/affaan-m_ECC.md) | agent | JavaScript | 191,141 | agent, coding-agent, harness |
| [hermes-agent](starred/NousResearch_hermes-agent.md) | agent | Python | 166,044 | agent, ai, llm |
| [yt-dlp](starred/yt-dlp_yt-dlp.md) | unknown | Python | 165,420 | audio, ide, python |
| [opencode](starred/anomalyco_opencode.md) | agent | TypeScript | 164,894 | agent |
| [FreeDomain](starred/DigitalPlatDev_FreeDomain.md) | unknown | HTML | 164,764 | ai |
| [andrej-karpathy-skills](starred/multica-ai_andrej-karpathy-skills.md) | unknown | other | 152,794 | llm, research |
| [langflow](starred/langflow-ai_langflow.md) | agent | Python | 148,729 | agent, ai |
| [open-webui](starred/open-webui_open-webui.md) | unknown | Python | 138,514 | ai, api, llm |
| [firecrawl](starred/firecrawl_firecrawl.md) | agent | TypeScript | 123,886 | agent, ai, llm |
| [agency-agents](starred/msitarzewski_agency-agents.md) | agent | Shell | 104,836 | agent, ai |
| [skills](starred/mattpocock_skills.md) | unknown | Shell | 103,989 | ai |
| [gstack](starred/garrytan_gstack.md) | unknown | TypeScript | 101,843 |  |
| [angular](starred/angular_angular.md) | webapp | TypeScript | 100,135 | ide, javascript, typescript |
| [awesome-design-md](starred/VoltAgent_awesome-design-md.md) | agent | other | 83,696 | agent |
| [autoresearch](starred/karpathy_autoresearch.md) | agent | Python | 83,168 | agent, ai, training |
| [netdata](starred/netdata_netdata.md) | unknown | C | 78,932 | ai, docker, multi-agent |

*...and 127 more in `starred/`*

## Tag Index

| Tag | Count | Repos |
|---|---|---|
| `agent` | 89 | hermes-web-computer, hermes-sync, repo-transmute +86 more |
| `ai` | 96 | hermes-web-computer, hermes-sync, repo-transmute +93 more |
| `aie` | 6 | seans-reporepo, seans, ecosystem +3 more |
| `analytics` | 2 | features-list, seans |
| `api` | 20 | hermes-sync, hermes-computer-planning, agent-os +17 more |
| `audio` | 3 | hermes-web-computer, Fun-Audio-Chat, yt-dlp |
| `automation` | 13 | hermes-computer-planning, seans-reporepo, hermes-agent +10 more |
| `awesome-list` | 16 | hermes-computer-planning, features-list, seans-reporepo +13 more |
| `backup` | 8 | hermes-sync, seans-reporepo, seans +5 more |
| `benchmarking` | 2 | llm-benchmark-platform, cua |
| `bot` | 13 | hermes-webui, hermes-computer-planning, features-list +10 more |
| `browser-automation` | 15 | hermes-web-computer, hermes-bootstrap, hermes-webui +12 more |
| `cli` | 23 | hermes-web-computer, repo-transmute, hermes-webui +20 more |
| `code-packing` | 2 | seans-reporepo, repomix |
| `coding-agent` | 3 | oh-my-openagent, ECC, OpenHands |
| `dashboard` | 8 | hermes-sync, everything-dashboard, features-list +5 more |
| `database` | 2 | agent-os, linux-web-serving-infrastructure |
| `docker` | 18 | hermes-web-computer, hermes-sync, hermes-bootstrap +15 more |
| `dotfiles` | 3 | sean-dotfiles, mod-illogical-impulse-dotfiles, dotfiles |
| `embeddings` | 1 | arcadedb |
| `energy` | 3 | seans-reporepo, seans, energy-aware-task-router |
| `express` | 2 | everything-dashboard, agent-os |
| `framework` | 15 | repo-transmute, hermes-webui, hermes-computer-planning +12 more |
| `gaming` | 9 | seans-reporepo, seans, starcraft-battlenet-web +6 more |
| `go` | 26 | hermes-web-computer, hermes-computer-planning, agent-os +23 more |
| `harness` | 8 | seans-reporepo, seans, claw-aie +5 more |
| `hermes-agent` | 13 | hermes-web-computer, hermes-sync, hermes-bootstrap +10 more |
| `ide` | 22 | repo-transmute, hermes-webui, hermes-computer-planning +19 more |
| `image-gen` | 2 | circuit-breaker-framework, rasta-assistant |
| `infrastructure` | 8 | seans-reporepo, seans, hermes-agent +5 more |
| `javascript` | 5 | webdriverio, Front-End-Checklist, repomix +2 more |
| `llm` | 37 | repo-transmute, agent-os, seans-reporepo +34 more |
| `logging` | 1 | linux-web-serving-infrastructure |
| `mcp` | 10 | hermes-computer-planning, features-list, nanobot +7 more |
| `mobile` | 5 | features-list, seans-reporepo, linux-web-serving-infrastructure +2 more |
| `monitoring` | 5 | hermes-computer-planning, circuit-breaker-framework, starcraft-battlenet-web +2 more |
| `multi-agent` | 18 | repo-transmute, features-list, seans +15 more |
| `nginx` | 5 | casaos-agent, starcraft-battlenet-web, linux-web-serving-infrastructure +2 more |
| `openclaw` | 10 | seans, nanobot, secureclaw +7 more |
| `orchestration` | 10 | features-list, seans, circuit-breaker-framework +7 more |
| `plugin` | 3 | secureclaw, vinext, compound-engineering-plugin |
| `proxy` | 4 | starcraft-battlenet-web, linux-web-serving-infrastructure, mobile-codeovertcp.com +1 more |
| `python` | 20 | repo-transmute, hermes-webui, hermes-computer-planning +17 more |
| `rag` | 8 | circuit-breaker-framework, casaos-agent, minsky-circuit +5 more |
| `react` | 14 | repo-transmute, everything-dashboard, features-list +11 more |
| `reliability` | 2 | seans, circuit-breaker-framework |
| `remote-dev` | 2 | linux-web-serving-infrastructure, mobile-codeovertcp.com |
| `research` | 1 | andrej-karpathy-skills |
| `rust` | 8 | seans, g3kilocode, postflop-solver +5 more |
| `solver` | 5 | repo-transmute, hermes-agent, starcraft-battlenet-web +2 more |
| `sustainability` | 1 | energy-aware-task-router |
| `sync` | 7 | hermes-web-computer, hermes-sync, hermes-bootstrap +4 more |
| `telemetry` | 3 | hermes-web-computer, langfuse, openclaw-telemetry |
| `terminal` | 10 | hermes-web-computer, hermes-webui, seans +7 more |
| `testing` | 3 | linux-web-serving-infrastructure, kali-openclaw-usb, promptfoo |
| `training` | 4 | energy-aware-task-router, DrMAS, RustTraining +1 more |
| `transpilation` | 5 | repo-transmute, features-list, seans-reporepo +2 more |
| `tui` | 4 | hermes-bootstrap, hermes-agent, pokerkit +1 more |
| `typescript` | 15 | repo-transmute, hermes-computer-planning, seans-reporepo +12 more |
| `vector-db` | 4 | memsearch, openpencil, arcadedb +1 more |
| `vector-search` | 2 | repo-transmute, claw-aie-harness |
| `video` | 1 | yt-dlp |
| `voice` | 6 | hermes-web-computer, seans-reporepo, hermes-agent +3 more |
| `vpn` | 1 | hermes-computer-planning |
| `web-app` | 33 | hermes-web-computer, hermes-bootstrap, hermes-webui +30 more |
| `website` | 1 | sean-s-landing-page |
| `workflow-engine` | 2 | hermes-web-computer, temporal |

## Language Breakdown

| Language | Count | Total Size |
|---|---|---|
| TypeScript | 57 | 9,370,666 KB |
| Python | 47 | 4,235,720 KB |
| other | 20 | 152,801 KB |
| Shell | 13 | 638,012 KB |
| Go | 12 | 823,710 KB |
| Rust | 9 | 290,726 KB |
| HTML | 5 | 223,536 KB |
| JavaScript | 5 | 180,607 KB |
| Nix | 4 | 295,047 KB |
| C++ | 3 | 641,777 KB |
| C | 3 | 284,978 KB |
| Jupyter Notebook | 2 | 649,626 KB |
| Java | 2 | 99,155 KB |
| QML | 1 | 998 KB |
| CSS | 1 | 180 KB |
| Vue | 1 | 4,450 KB |
| C# | 1 | 723 KB |
| Dockerfile | 1 | 1,103 KB |
| PHP | 1 | 51,161 KB |

## Combinatorial Potential

Repos that share tags are candidates for combination:

### `agent`
- Owned: hermes-web-computer, hermes-sync, repo-transmute
- Starred: php-modernization-skill, skills, skills

### `ai`
- Owned: hermes-web-computer, hermes-sync, repo-transmute
- Starred: skills, skills, browser-agent-gepa

### `api`
- Owned: hermes-sync, hermes-computer-planning, agent-os
- Starred: mcp2cli, open-webui, vinext

### `audio`
- Owned: hermes-web-computer
- Starred: Fun-Audio-Chat, yt-dlp

### `automation`
- Owned: hermes-computer-planning, seans-reporepo, hermes-agent
- Starred: browser-agent-gepa, kernel-images, webdriverio

### `awesome-list`
- Owned: hermes-computer-planning, features-list, seans-reporepo
- Starred: Front-End-Checklist, awesome, awesome-frontend-resources

### `benchmarking`
- Owned: llm-benchmark-platform
- Starred: cua

### `bot`
- Owned: hermes-webui, hermes-computer-planning, features-list
- Starred: bytebot, docker-swag, nanobot-webgui

### `browser-automation`
- Owned: hermes-web-computer, hermes-bootstrap, hermes-webui
- Starred: browser-agent-gepa, kernel-images, webdriverio

### `cli`
- Owned: hermes-web-computer, repo-transmute, hermes-webui
- Starred: autify-cli, docker-swag, awesome-agent-skills

### `code-packing`
- Owned: seans-reporepo
- Starred: repomix

### `dashboard`
- Owned: hermes-sync, everything-dashboard, features-list
- Starred: opik, openclaw-mission-control

### `docker`
- Owned: hermes-web-computer, hermes-sync, hermes-bootstrap
- Starred: kernel-images, bytebot, arcadedb

### `dotfiles`
- Owned: sean-dotfiles
- Starred: mod-illogical-impulse-dotfiles, dotfiles

### `framework`
- Owned: repo-transmute, hermes-webui, hermes-computer-planning
- Starred: hypeman, webdriverio, design-resources-for-developers

